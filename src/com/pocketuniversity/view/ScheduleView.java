		...
		if (classList != null && classList.size() > 0) {
			...
			for (int i = 0; i < classList.size(); i++) {
				...
				int fromX = startX + sidewidte + eachBoxW
						* (Integer.parseInt(classInfo.getWeekday()) - 1);
				int fromY = startY + sidewidte + eachBoxH
						* (Integer.parseInt(classInfo.getFromClassNum()) - 1);
				int toX = startX + sidewidte + eachBoxW
						* Integer.parseInt(classInfo.getWeekday());
				int toY = startY
						+ sidewidte
						+ eachBoxH
						* (Integer.parseInt(classInfo.getFromClassNum())
								+ Integer.parseInt(classInfo.getClassNumLen()) - 1);
