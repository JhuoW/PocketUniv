...
public class AbImageUtil {
	 ...
	 *            要下载文件的网络地址
	 * @param type
	 *            图片的处理类型（剪切或�?�缩放到指定大小，参考AbConstant类）
	 * @param desiredWidth
	 *            新图片的�?
	 * @param desiredHeight
	 *            新图片的�?
	...
	public static Bitmap scaleImg(File file, int desiredWidth, int desiredHeight) {
		...
		// 以下两个字段�?�?起使用：
		// 产生的位图将得到像素空间，如果系统gc，那么将被清空�?�当像素再次被访问，如果Bitmap已经decode，那么将被自动重新解�?
		opts.inPurgeable = true;
		// 位图可以共享�?个参考输入数�?(inputstream、阵列等)
		opts.inInputShareable = true;

		// inSampleSize=2 表示图片宽高都为原来的二分之�?，即图片为原来的四分之一
		// 缩放的比例，SDK中建议其值是2的指数�?�，通过inSampleSize来进行缩放，其�?�表明缩放的倍数
		if (scale < 0.25) {
			// 缩小�?4分之�?
			opts.inSampleSize = 2;
		} else if (scale < 0.125) {
	...
	public static Bitmap scaleImg(Bitmap bitmap, float scale) {
		...
		try {
			...
			// 注意这个Matirx是android.graphics底下的那�?
			Matrix matrix = new Matrix();
			// 设置缩放系数，分别为原来�?0.8�?0.8
	...
	public static Bitmap cutImg(File file, int desiredWidth, int desiredHeight) {
		...
		// 位图可以共享�?个参考输入数�?(inputstream、阵列等)
		opts.inInputShareable = true;
		// 缩放的比例，缩放是很难按准备的比例进行缩放的，�?�过inSampleSize来进行缩放，其�?�表明缩放的倍数，SDK中建议其值是2的指数�??
		if (scale < 0.25) {
			// 缩小�?4分之�?
			opts.inSampleSize = 2;
		} else if (scale < 0.125) {
	...
	/**
	 * 将View转换为Drawable.�?要最上层布局为Linearlayout
	 * 
	 * @param view
	 *            要转换为Drawable的View
	...
	/**
	 * 将View转换为Bitmap.�?要最上层布局为Linearlayout
	 * 
	 * @param view
	 *            要转换为bitmap的View
	 * @return byte[] 图片的byte[]
	...
	public static String getHashCode(Bitmap bitmap) {
		// 第一步，缩小尺寸�?
		// 将图片缩小到8x8的尺寸，总共64个像素�?�这�?步的作用是去除图片的细节�?
		...
		Log.i("th", "将图片缩小到8x8的尺�?:" + width + "*" + height);

		// 第二步，第二步，�?化色彩�??
		// 将缩小后的图片，转为64级灰度�?�也就是说，�?有像素点总共只有64种颜色�??
		...
		// 第四步，比较像素的灰度�??
		// 将每个像素的灰度，与平均值进行比较�?�大于或等于平均值，记为1；小于平均�?�，记为0�?
		...
		// 得到指纹以后，就可以对比不同的图片，看看64位中有多少位是不�?样的�?
		// 在理论上，这等同于计�?"汉明距离"（Hamming distance）�??
	...
	public static String getDCTHashCode(Bitmap bitmap) {
		...
		int width = temp.getWidth();
		int height = temp.getHeight();
		Log.i("th", "将图片缩小到32x32的尺�?:" + width + "*" + height);
	...
	/**
	 * 描述：图像的特征值颜色分�? 将颜色分4个区�?0,1,2,3 区组合共64组，计算每个像素点属于哪个区.
	 * 
	 * @param bitmap
	 *            the bitmap
