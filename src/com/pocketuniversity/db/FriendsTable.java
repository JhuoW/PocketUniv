...
import com.pocketuniversity.utils.ParcelableUtil;
...
public class FriendsTable {
	...
	public List<PUUser> selectFriends() {
		List<PUUser> friends = new ArrayList<PUUser>();
		SQLiteDatabase db = dbHelper.getReadableDatabase();
		Cursor c = db.rawQuery("SELECT * FROM friends", null);
		while (c.moveToNext()) {
			PUUser friend = createFriendByCursor(c);
			friends.add(friend);
		}
		c.close();
		return friends;
	...
	static PUUser createFriendByCursor(Cursor c) {
		byte[] msgBytes = c.getBlob(c.getColumnIndex(OBJECT));
		if (msgBytes != null) {
			PUUser user = (PUUser) ParcelableUtil.unmarshall(msgBytes,
					PUUser.CREATOR);
			return user;
		} else {
			return null;
	...
	public void insertFriends(List<PUUser> friends) {
		SQLiteDatabase db = dbHelper.getWritableDatabase();
		db.beginTransaction();
		try {
			for (PUUser friend : friends) {
	 ...
	 * @return
	 */
	public byte[] marshallMsg(PUUser user) {
	...
	public void deleteAllFriend(List<PUUser> friends) {
		SQLiteDatabase db = dbHelper.getWritableDatabase();
		for (PUUser friend : friends) {
