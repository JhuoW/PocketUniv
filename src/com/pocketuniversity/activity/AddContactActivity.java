...
public class AddContactActivity extends BaseActivity {
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		String QRFriendCode = intent.getStringExtra("QRFriendCode");

		mTextView = (TextView) findViewById(R.id.add_list_friends);

		editText = (EditText) findViewById(R.id.edit_note);
		
		if (QRFriendCode != null) {
			editText.setText(QRFriendCode);
		}
	...
	private Handler handler = new Handler() {
		...
		public void handleMessage(android.os.Message msg) {
			...
			final List<PUUser> list = GetObjectFromService
					.getSearchResult(jsonStr);

			int j = list.size();

			for (int i = 0; i < j; i++) {
				String nickname = list.get(i).getName();
				String url = list.get(i).getImage();
				toAddUsername = nickname;
				imageLoader.displayImage(url, avatar,PhotoUtils.getImageOptions(R.drawable.icon_default_avatar));
	...
	public void searchContact(View v) {
		...
		new Thread(new Runnable() {
			...
			public void run() {
				...
				String jsonStr = new WebService(C.GETSEARCHLIST, param)
						.getReturnInfo();
				Log.i("AddContactActivity", jsonStr);
		...
		// }
		//
		// // TODO 从服务器获取此contact,如果不存在提示不存在此用�?
		//
		// //服务器存在此用户，显示此用户和添加按�?
		// searchedUserLayout.setVisibility(View.VISIBLE);
		// nameText.setText(toAddUsername);
