...
public class MessageAdapter extends BaseAdapter{
	...
	public MessageAdapter(Context context, String username, int chatType , String userId ) {
	...
	public MessageAdapter(Context context , String userId ,String groupId){
		this.context = context;
		this.groupId = groupId;
		this.conversation = EMChatManager.getInstance().getConversation(groupId);
	}
	...
	public View getView(final int position, View convertView, ViewGroup parent) {
		...
		if ((chatType == ChatType.GroupChat || chatType == chatType.ChatRoom) && message.direct == EMMessage.Direct.RECEIVE){
			...
			try {
				//拓展消息中添加姓名和头像
				nameExpand = message.getStringAttribute("name").toString();
				imgUrlExpand = message.getStringAttribute("imgUrl").toString();
                Log.i("name + imgUrl", "name :"+nameExpand + "; imgUrl:" + imgUrlExpand);
			} catch (EaseMobException e) {
				// TODO Auto-generated catch block
				e.printStackTrace();
			}
			holder.tv_usernick.setText(groupDBHelper.getNicknameByid(message.getFrom()));
			//holder.tv_usernick.setText(message.getFrom());
			setGroupUserAvatar(message, holder.iv_avatar);
			setGroupUserAvatarByExpandMessage(message,imgUrlExpand, nameExpand ,holder.iv_avatar);
		}else {
			try {
				nameExpand = message.getStringAttribute("name").toString();
				imgUrlExpand = message.getStringAttribute("imgUrl").toString();
			...
			setSingleAvatarByExpandMessage(message,imgUrlExpand, holder.iv_avatar);
		}
...
//		setUserAvatar(message, holder.iv_avatar);
	...
	private void setUserAvatar(EMMessage message, ImageView imageView){
	    if(message.direct == Direct.SEND){
	    	System.out.println("当前用户编号：" + Utils.getID());
	        //显示自己头像
	        UserUtils.setUserAvatar2(context, imageView);
	    }else{
	    	System.out.println("发送者头像"+userId);
	        UserUtils.setUserAvatar4(context, message.getFrom(), imageView);
	    }
	}
	
	private void setGroupUserAvatar(EMMessage message, ImageView imageView){
	    if(message.direct == Direct.SEND){
	    	System.out.println("当前用户编号：" + Utils.getID());
	        //显示自己头像
	        UserUtils.setUserAvatar2(context, imageView);
	    	...
	    	System.out.println("发送者头像"+userId);
	        UserUtils.setUserAvatar5(context, message.getFrom(), imageView);
	    }
	}
	 ...
	 * 通过拓展消息设置群成员头像 和昵称
	 */
	private void setGroupUserAvatarByExpandMessage(EMMessage message ,String imgUrl , String userName , ImageView imgView){
	    if(message.direct == Direct.SEND){
	        UserUtils.setUserAvatar2(context, imgView);
	    }else{
	 ...
	 * 通过拓展消息设置单聊头像
	 */
	private void setSingleAvatarByExpandMessage(EMMessage message,String imgUrl , ImageView imgView){
		 if(message.direct == Direct.SEND){
		        UserUtils.setUserAvatar2(context, imgView);
		    }else{
	   	ImageLoader imageLoader = ImageLoader.getInstance();
